...
class PropertyHierarchy:
    ...
    def __init__(self, onto):
        self.all_properties = self.parse(onto.properties())
        self.data_properties = self.parse(onto.data_properties())
        self.object_properties = self.parse(onto.object_properties())

    @staticmethod
    def parse(selected_properties):
        result = set()
        for i in selected_properties:
            relations = []
            for s, o in i.get_relations():
                s_iri = s.iri
                if hasattr(o, 'iri'):
                    o_iri = o.iri
                else:
                    o_iri = str(o)
                relations.append((s_iri, o_iri))

            p = Property(name=i.name, relations=relations)
            result.add(p)
        return result
...
class Experience:
    ...
    def append(self, e):
        ...
        try:
            ...
            print(s_i, '\t', type(s_i.embeddings))
            print(s_j, '\t', type(s_j.embeddings))
